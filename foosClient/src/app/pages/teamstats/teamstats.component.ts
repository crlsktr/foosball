...
export class TeamStatsComponent implements OnInit {
	...
	private loadStats() {
		this.foosService
			...
			.then((stats: TeamStat[]) => {
				this.stats = stats;
				this.drawHistory(
					this.stats.map<HistoryPoint>(x => ({
						played: new Date(x.played_on),
						ranking: x.current_ranking,
					}))
				);
			})
			.catch(err => {
	...
	drawHistory(hist: HistoryPoint[]) {
		...
		let xScale = d3
			...
			.range([
				this.margin.left + this.axisWidth.y,
				this.width - this.margin.right,
			]);
		let yScale = d3
			.scaleLinear()
			.domain([
				d3.min(hist, x => x.ranking),
				d3.max(hist, x => x.ranking),
			])
			.nice()
			.range([
				this.height - (this.margin.bottom + this.axisWidth.x),
				this.margin.top,
			]);
			...
			g
				.attr(
					...
					`translate(0, ${this.height -
						(this.margin.bottom + this.axisWidth.x)})`
				)
				.call(
					d3
						.axisBottom(xScale)
		...
		let yAxis = (g: d3.Selection<d3.BaseType, {}, HTMLElement, any>) =>
			g
				.attr(
					'transform',
					`translate(${this.margin.left + this.axisWidth.y},0)`
				)
	...
	hideGame(d: HistoryPoint) {
		...
		let game = this.stats.find(
			x =>
				new Date(x.played_on).toLocaleString() ===
				d.played.toLocaleString()
		);
		game.highlight = true;
